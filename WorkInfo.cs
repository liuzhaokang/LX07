...
namespace DAL
{
    public  class WorkInfo
    {
        ...
        public Model.WorkInfo GetModel(int id)
        {
            ...
            {
                string sql = "sleect workinfo.*,activity.activityName from workinfo join activity on workinfo.activityId=activity.activityId where workId=@id";
        ...
        public IEnumerable<Model.WorkInfoNo>GetFindPage(Model.WorkFindPage page)
        {
            using (IDbConnection cn =new MySqlConnection(cns))
            {
                ...
                sql += "select* from a where num between (@pageIndex-1)*@pageSize+1 and @pageIdex*@pageSize;";
        ...
        public int Add(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into workinfo (workId,workName,workPicture,uploadTime,workIntroduciton,workVerify,userName,activityId,recommend,recommendTime)" + "values(@workId,@workName.@workPicture,@uploadTime,@workIntroduction,@workVerify,@userName,@activityId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int update(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName=@workName,workPicture=@workPicture,uploadTime=@uploadTime,workIntroduciton=@workIntroduciton,workVerify=@workVerify," + "userName=@userName,recommend=@recommend,recommendTime=@recommendTime where workId=@workId";
                return cn.Execute(sql, workinfo);
            }
        }
        public int UpdateTime(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
